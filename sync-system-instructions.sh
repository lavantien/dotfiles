...
#   -c             Commit changes using Claude CLI
...
while getopts "d:cp" opt; do
    case $opt in
        ...
        c) COMMIT=true ;;
        p) PUSH=true ;;
        *) echo "Usage: $0 [-d base_dir] [-c] [-p]" >&2; exit 1 ;;
    esac
done
...
commit_with_claude() {
    if ! command -v claude >/dev/null 2>&1; then
        echo -e "${YELLOW}Claude CLI not found - skipping${NC}"
        return 0
    fi

    echo -e "${BLUE}Committing system instructions via Claude CLI...${NC}"
...
if [[ "$COMMIT" == true ]]; then
    ...
    if command -v claude >/dev/null 2>&1 && [[ "$PUSH" == true ]]; then
