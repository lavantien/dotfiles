...
```bash
sudo apt update && sudo apt upgrade -y && sudo apt autoremove -y && sudo apt install ca-certificates apt-transport-https ubuntu-dev-tools glibc-source gcc xclip git git-lfs curl zsh htop neofetch vim libutf8proc2 libutf8proc-dev libfuse2 cpu-checker cmake cmake-format ninja-build libjsoncpp-dev uuid-dev zlib1g-dev libssl-dev jq bc x11-apps libx11-dev xserver-xorg-dev xorg-dev libasound2-dev libudev-dev libxcursor-dev libgl1-mesa-dev cloud-init openssh-server ssh-import-id -y
```
...
```bash
pip3 install cmake-language-server python-lsp-server && npm install --global sql-formatter && sudo apt install python-is-python3 -y && go install github.com/charmbracelet/glow@latest && go install github.com/fatih/gomodifytags@latest && go install -v github.com/incu6us/goimports-reviser/v3@latest
```
...
### 16. Install `Cuda` inside `WSL`

```bash
wget https://developer.download.nvidia.com/compute/cuda/repos/wsl-ubuntu/x86_64/cuda-wsl-ubuntu.pin && sudo mv cuda-wsl-ubuntu.pin /etc/apt/preferences.d/cuda-repository-pin-600 && wget https://developer.download.nvidia.com/compute/cuda/12.1.1/local_installers/cuda-repo-wsl-ubuntu-12-1-local_12.1.1-1_amd64.deb && sudo dpkg -i cuda-repo-wsl-ubuntu-12-1-local_12.1.1-1_amd64.deb && sudo cp /var/cuda-repo-wsl-ubuntu-12-1-local/cuda-*-keyring.gpg /usr/share/keyrings/ && sudo apt update && sudo apt -y install cuda && rm cuda-repo-wsl-ubuntu-12-1-local_12.1.1-1_amd64.deb
```
